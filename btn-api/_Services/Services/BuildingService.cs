...
namespace btn_api._Services.Services
{
    public class BuildingService : IBuildingService
    {
        ...
        public async Task<object> GetBuildingAsTreeView()
        {
           return await _repoBuilding.FindAll().Where(x=>x.Level != 1 ).ToListAsync();
        }

        public async Task<List<BuildingDto>> GetLines()
        {
            var WORKER_LEVEL = 5;
            return await _repoBuilding.FindAll().Where(x => x.Level == WORKER_LEVEL).ProjectTo<BuildingDto>(_configMapper).OrderByDescending(x => x.ID).ToListAsync();
        }
    }
}